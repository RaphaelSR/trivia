  ...
  return (
    <aside className="sidebar-surface flex h-full flex-col p-2.5">
      <div className="mb-2 flex items-center justify-between gap-2 border-b border-white/8 pb-2">
        {!collapsed ? (
          <p className="truncate text-xs font-semibold text-[var(--color-muted)]">{title}</p>
        ) : null}
        <Button variant="ghost" size="icon" onClick={onToggleCollapsed} aria-label="Alternar menu lateral" className="ml-auto">
          <Menu size={14} />
        </Button>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-[var(--color-overlay)]/95 xl:hidden" onClick={props.onCloseMobile}>
          <div className="absolute inset-y-0 left-0 w-[80vw] max-w-[280px] p-2" onClick={(e) => e.stopPropagation()}>
            <div className="sidebar-surface flex h-full flex-col rounded-2xl p-2.5">
              <div className="mb-2 flex items-center justify-between gap-2 border-b border-white/8 pb-2">
                <p className="truncate text-xs font-semibold text-[var(--color-muted)]">{props.title}</p>
                <Button variant="ghost" size="icon" onClick={props.onCloseMobile} aria-label="Fechar menu lateral">
                  <X size={14} />
                </Button>
              </div>
