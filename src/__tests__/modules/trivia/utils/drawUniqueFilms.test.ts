...
describe('drawUniqueFilms', () => {
  ...
  const createParticipants = (names: string[], teamName: string) => {
  ...
  describe('Cenário 1: 15 jogadores, cada um indica 1 filme', () => {
    ...
    const allParticipants = [
      ...teamANames.map((name, i) => ({ name, id: `participant-a-${i}`, teamName: 'Time A' })),
      ...teamBNames.map((name, i) => ({ name, id: `participant-b-${i}`, teamName: 'Time B' })),
      ...teamCNames.map((name, i) => ({ name, id: `participant-c-${i}`, teamName: 'Time C' }))
    ]
  ...
  describe('Cenário 2: 15 jogadores, cada um indica 3 filmes', () => {
    ...
    it('deve sortear 4 filmes únicos, descartando outros filmes de pessoas que já tiveram 1', () => {
      ...
      const personNames = results.map(r => r.participantName.toLowerCase().trim())
    ...
    it('com allowMultiplePerPerson=true, deve permitir múltiplos filmes da mesma pessoa', () => {
      ...
      const hasMultiple = Array.from(personCounts.values()).some(count => count > 1)
      
      if (!hasMultiple) {
        const results2 = drawUniqueFilms(films, participantsMap, 10, true)
        const personNames2 = results2.map(r => r.participantName.toLowerCase().trim())
        const personCounts2 = new Map<string, number>()
        personNames2.forEach(name => {
          personCounts2.set(name, (personCounts2.get(name) || 0) + 1)
        })
        const hasMultiple2 = Array.from(personCounts2.values()).some(count => count > 1)
  ...
  describe('Cenário 3: Filmes com títulos duplicados', () => {
    ...
    it('deve sortear no máximo 1 "Matrix" mesmo sendo indicado por duas pessoas', () => {
      for (let i = 0; i < 10; i++) {
        ...
        const matrixCount = results.filter(r => 
          r.film.name.toLowerCase().trim() === 'matrix'
        ).length
  ...
  describe('Casos extremos', () => {
    ...
    it('deve retornar array vazio se maxFilms é 0', () => {
      const participantsMap = new Map([['p1', { name: 'João', teamName: 'Time A' }]])
