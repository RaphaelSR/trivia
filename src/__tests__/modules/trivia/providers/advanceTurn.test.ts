...
describe('advanceTurn - Wrap-around e Regeneração Automática', () => {
  const createParticipant = (id: string, name: string, teamId: string): TriviaParticipant => ({
    id,
    name,
    role: 'player',
    teamId,
  ...
  const hasConsecutiveSameTeam = (sequence: string[], participants: TriviaParticipant[]): boolean => {
  ...
  describe('Regeneração após wrap-around', () => {
    it('deve regenerar sequência quando completa uma rodada (3 times 4-3-3)', () => {
      ...
      expect(initialSequence).toEqual(['A1', 'B1', 'C1', 'A2', 'B2', 'C2', 'A3', 'B3', 'C3', 'A4'])
      ...
      const lastParticipant = participants.find(p => p.id === lastParticipantId)
      ...
      const firstParticipant = participants.find(p => p.id === regeneratedSequence[0])
      ...
      const newFirstParticipant = participants.find(p => p.id === finalSequence[0])
