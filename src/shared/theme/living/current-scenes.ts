import {
  ...
  seededRandom,
  type Particle,
  type ProceduralSceneState,
} from './core'
import {
  CURRENT_LIVING_THEME_IDS,
  type LivingSceneRenderer,
  type LivingSceneViewport,
} from './types'

type CurrentLivingTheme = (typeof CURRENT_LIVING_THEME_IDS)[number]
type SceneData = ProceduralSceneState

function resizeParticleSet(
  particles: Particle[],
  count: number,
  random: () => number,
) {
  if (particles.length > count) particles.length = count
    ...
    particles.push(...createParticles(count - particles.length, random))
  }
}

function resizeSceneData(
  data: SceneData,
  viewport: LivingSceneViewport,
  random: () => number,
) {
  resizeParticleSet(data.particles, Math.round(90 * viewport.density), random)
  resizeParticleSet(data.secondary, Math.round(38 * viewport.density), random)
}

export function createCurrentSceneRenderer(
  theme: CurrentLivingTheme,
): LivingSceneRenderer {
  return {
    create({ viewport, seed, random }) {
      ...
      return {
        resize(nextViewport) {
          resizeSceneData(data, nextViewport, random)
        },
        update() {},
...
function drawWebCity(
  ...
  data.particles.slice(0, 22).forEach((particle) => {
    const pulse =
      0.35 + Math.sin(time * particle.speed * 2 + particle.phase) * 0.2
    context.fillStyle = `rgba(${particle.x > 0.5 ? '248,72,72' : '65,124,255'},${pulse})`
    context.fillRect(
      particle.x * width,
      particle.y * height * 0.72,
      particle.size,
      particle.size,
    )
  })
...
function drawComicSky(
  ...
  for (let y = 0; y < height * 0.58; y += spacing) {
    for (
      let x = (y / spacing) % 2 === 0 ? 0 : spacing / 2;
      x < width;
      x += spacing
    ) {
      context.beginPath()
...
function drawDeepSpace(
  ...
  data.particles.forEach((particle) => {
    const x =
      ((particle.x + time * particle.speed * 0.002 * particle.depth) % 1) *
      width
    const y = particle.y * height
    const alpha =
      0.4 + Math.sin(time * (0.7 + particle.speed) + particle.phase) * 0.28
...
function drawCinemaFacade(
  ...
  for (let bulb = 0; bulb < bulbCount; bulb += 1) {
    const x =
      centerX -
      facadeWidth * 0.39 +
      (bulb / (bulbCount - 1)) * facadeWidth * 0.78
...
function drawBats(
  ...
  for (let bat = 0; bat < 5; bat += 1) {
    ...
    const y =
      height * (0.12 + (bat % 3) * 0.07 + Math.cos(time * 0.45 + bat) * 0.018)
...
function drawFilmScratches(
  ...
  data.secondary.slice(0, 6).forEach((scratch, index) => {
    const x =
      ((scratch.x + Math.floor(time * (2 + index * 0.2)) * 0.17) % 1) * width
    context.beginPath()
    context.moveTo(x, 0)
...
function drawUnderwater(
  ...
  data.particles.slice(0, 46).forEach((bubble) => {
    ...
    const y =
      height * (1.08 - ((bubble.y + time * bubble.speed * 0.025) % 1.16))
...
function drawJellyfish(
  ...
  jellyfish.slice(0, 7).forEach((jelly, index) => {
    ...
    const y =
      height * (0.16 + jelly.y * 0.53) +
      Math.cos(time * 0.21 + jelly.phase) * 16
    ...
    glow.addColorStop(
      0,
      index % 2 ? 'rgba(133,255,231,0.3)' : 'rgba(170,137,255,0.28)',
    )
    ...
    context.fill()
    context.fillStyle =
      index % 2 ? 'rgba(124,241,220,0.34)' : 'rgba(179,142,255,0.31)'
    context.beginPath()
...
function drawStadiumLife(
  ...
  data.particles.slice(0, 70).forEach((spectator, index) => {
    ...
    const flash =
      Math.sin(time * (0.8 + spectator.speed) + spectator.phase) > 0.94
  ...
  for (let flag = 0; flag < 8; flag += 1) {
    ...
    context.stroke()
    context.fillStyle = argentina
      ? 'rgba(111,178,224,0.42)'
      : flag % 2
        ? 'rgba(205,37,51,0.44)'
        : 'rgba(247,196,69,0.44)'
    context.beginPath()
    context.moveTo(x, y - 30)
    context.quadraticCurveTo(
      x + 18,
      y - 36 + Math.sin(time * 2 + flag) * 4,
      x + 34,
      y - 28,
    )
...
function drawKawaiiLife(
  ...
  data.secondary.slice(0, 22).forEach((item, index) => {
    ...
    context.fillStyle =
      index % 2 ? 'rgba(244,114,182,0.2)' : 'rgba(124,58,237,0.16)'
    if (index % 3 === 0) {
      context.beginPath()
...
function drawNeonLife(
  ...
  data.secondary.slice(0, 7).forEach((vehicle, index) => {
    ...
    gradient.addColorStop(
      0,
      index % 2 ? 'rgba(255,79,216,0.56)' : 'rgba(39,244,210,0.56)',
    )
